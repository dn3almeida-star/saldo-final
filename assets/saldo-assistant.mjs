...
export function createAssistantClient({
  ...
  async function handleResponse(response) {
    ...
    if (typeof body?.transcript === 'string' && body.transcript) setState({ transcript: body.transcript });
  ...
  async function sendJson(text) {
    const token = await preflight();
    if (!token) return cloneState(state);
    setState({ status: 'submitting', message: 'Analisando seu pedido...', errorCode: '', reportOutput: '' });
    try {
      const response = await fetchImpl(endpoint, {
        method: 'POST',
